...
export const SkillCard: React.FC<ISkillCardProps> = ({ skillCard }: ISkillCardProps) => {
  ...
  const handleClick = () => {
    toggleChangeArea();
    setSearch(skillCard.name);
    setApplyFiltersBoolean(!applyFiltersBoolean);
  };

  return (
    <div className={`sm:touch-none select-none  cursor-pointer ${CARD_MARGIN_X} transition ease-in-out hover:scale-105 hover:shadow-md flex flex-col justify-evenly ${CARD_WIDTH} content-between items-center border rounded-2xl border-gray-300  ${skillCard.type === 'backend' ? 'h-[3rem] bg-[#f9f9f9]' : 'h-[3.575rem] bg-[#f7f7f7]'}`}
      onClick={ () => handleClick()}>
