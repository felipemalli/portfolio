      ...
      <div className='flex'>
        <div className={`${!isOpen ? 'hidden' : 'absolute'} w-screen md:w-auto md:flex md:relative right-1/2 translate-x-1/2 mt-12 md:mt-0 border-y md:border-none`}>
          <nav ref={navRef} className={'flex flex-col md:flex-row items-center text-center py-4 md:py-0 space-y-2 md:space-y-0 md:space-x-6 uppercase text-customBlue-500 tracking-widest font-semibold bg-secondary-900 md:bg-transparent'} onClick={() => setIsOpen(false)}>
            <NavOptionNavigation className='hover:scale-105 hover:bg-[#ebebeb] md:hover:bg-transparent px-2 w-screen h-10 md:w-auto flex items-center justify-center' componentId='content' areaNameNav='knowledge'>Knowledge</NavOptionNavigation>
            <NavOptionNavigation className='hover:scale-105 hover:bg-[#ebebeb] md:hover:bg-transparent px-2 w-screen h-10 md:w-auto flex items-center justify-center' componentId='content' areaNameNav='skills'>Skills</NavOptionNavigation>
            <NavOptionNavigation className='hover:scale-105 hover:bg-[#ebebeb] md:hover:bg-transparent px-2 w-screen h-10 md:w-auto flex items-center justify-center' componentId='about'>About</NavOptionNavigation>
            <NavOptionLink className='hover:scale-105 hover:bg-[#ebebeb] md:hover:bg-transparent px-2 w-screen h-10 md:w-auto flex items-center justify-center' url='https://www.linkedin.com/in/felipe-vahia-malliagros/'>Contact</NavOptionLink>
          </nav>
        </div>
        <button ref={navButton} type='button' className='h-10 w-10 block md:hidden  focus:outline-none hover:bg-[#ebebeb] rounded-full pl-2' onClick={() => setIsOpen(!isOpen)}>
          {isOpen 
