...
export const Header: React.FC = () => {
  ...
  return (
    <header id='header'>
      <div className='w-fill md:mx-8 lg:mx-16 2xl:w-[89.938rem] 2xl:m-auto mb-3'>
        <div className={`flex justify-between md:justify-center -mb-[88px] xl:justify-start p-6 md:my-4 xl:ml-20 2xl:ml-12 ${(isOpenNav || isOpenLanguage) && 'bg-secondary-500 md:bg-transparent'}`}>
          ...
          <div>
            <div className={`${!isOpenNav ? 'hidden' : 'absolute'} w-screen md:w-auto md:flex md:relative right-1/2 translate-x-1/2 mt-16 md:mt-0 border-y md:border-none`}>
              <nav ref={navRef} className={'flex flex-col md:flex-row items-center text-center py-4 md:py-0 space-y-2 md:space-y-0 md:space-x-8 2xl:space-x-10 uppercase text-customBlue-500 tracking-widest font-semibold bg-secondary-900 md:bg-transparent'} onClick={() => setIsOpenNav(false)}>
                ...
                {isMd && !isXl && <LanguageButton isOpenLanguage={isOpenLanguage} setIsOpenLanguage={setIsOpenLanguage} className='relative flex-col items-center gap-1 pb-0.5' />}
              </nav>
            </div>
            <div className='flex gap-4'>
              {!isMd && isRenderLanguage && <LanguageButton isOpenLanguage={isOpenLanguage} setIsOpenLanguage={setIsOpenLanguage} className='relative flex-col items-center gap-1'  setIsRenderLanguage={setIsRenderLanguage} />}
      ...
      {isXl && <LanguageButton isOpenLanguage={isOpenLanguage} setIsOpenLanguage={setIsOpenLanguage} hasText={true} className='absolute xl:right-[2.7%] 2xl:right-[3.4%] 3xl:right-[4.4%] top-[22px] flex-col items-center gap-1' />}
    </header>
  );
};
