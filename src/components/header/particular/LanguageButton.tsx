...
export const LanguageButton: React.FC<ILanguageButtonProps> = ({ className, hasText, setIsRenderLanguage }: ILanguageButtonProps) => {
  ...
  return (
    <div ref={languageRef} className={`${className} scale-90`}>
      <button className={`flex h-10 items-center ${hasText ? 'justify-end' : 'justify-center'} space-x-[5px] xl:w-full w-10 h-10 hover:bg-customGray-200 xl:p-2 rounded-full xl:rounded-none -ml-2 xl:ml-0 transition-colors duration-200`}
      ...
      <div className='absolute top-10 left-1/2 -translate-x-1/2 flex flex-col gap-1'>
        {isOpenLanguage && Object.entries(languageNames).map(([languageShort, languageName]) => (
          <button key={languageShort} className={`${languageShort === language && 'text-customGray-600 bg-secondary-500 cursor-not-allowed'} border-b hover:bg-customGray-200 bg-secondary-500 w-full px-2 pt-3 pb-2 font-medium text-customBlue-500 uppercase text-sm -mb-1 transition-colors duration-200`}
            onClick={() => {
